...
**Filter Syntax (CEL-based):**

The filter parameter uses CEL (Common Expression Language) syntax with `==` operators and logical operators (`&&`, `||`).

| Parameter              | Type    | Syntax                           | Example                                                |
| ---------------------- | ------- | -------------------------------- | ------------------------------------------------------ |
| `creator`              | String  | `creator == "users/{id}"`        | `creator == "users/1"`                                 |
| `visibilities`         | List    | `visibilities == ["V1", "V2"]`   | `visibilities == ["PUBLIC", "PROTECTED"]`              |
| `row_status`           | String  | `row_status == "STATUS"`         | `row_status == "NORMAL"` or `row_status == "ARCHIVED"` |
| `tag_search`           | List    | `tag_search == ["tag1", "tag2"]` | `tag_search == ["Raw/Pets", "Travel"]`                 |
| `content_search`       | List    | `content_search == ["term"]`     | `content_search == ["hello", "world"]`                 |
| `has_link`             | Boolean | `has_link == true`               | `has_link == true`                                     |
| `has_task_list`        | Boolean | `has_task_list == true`          | `has_task_list == true`                                |
| `has_code`             | Boolean | `has_code == true`               | `has_code == true`                                     |
| `has_incomplete_tasks` | Boolean | `has_incomplete_tasks == true`   | `has_incomplete_tasks == true`                         |
| `pinned`               | Boolean | `pinned == true`                 | `pinned == true`                                       |
| `display_time_before`  | Integer | Unix timestamp                   | `display_time_before == 1704067200`                    |
| `display_time_after`   | Integer | Unix timestamp                   | `display_time_after == 1704067200`                     |
| `order_by_pinned`      | Boolean | `order_by_pinned == true`        | `order_by_pinned == true`                              |
| `order_by_time_asc`    | Boolean | `order_by_time_asc == true`      | `order_by_time_asc == true`                            |
| `random`               | Boolean | `random == true`                 | `random == true`                                       |
| `limit`                | Integer | `limit == N`                     | `limit == 10`                                          |
| `include_comments`     | Boolean | `include_comments == true`       | `include_comments == true`                             |
| `uid`                  | String  | `uid == "memo-uid"`              | `uid == "abc123"`                                      |
...
tag_search == ["work", "important"]
content_search == ["keyword1", "keyword2"]
row_status == "NORMAL" && tag_search == ["Raw/Pets"] && has_code == true
display_time_before == 1735689599
display_time_after == 1704067200
has_link == true
has_task_list == true
has_code == true
has_incomplete_tasks == true
order_by_pinned == true && row_status == "NORMAL"
```

**Key Notes:**

- List values use square brackets: `["value1", "value2"]`
- String values use double quotes: `"value"`
- Boolean and integer values have no quotes: `true`, `false`, `10`
- Multiple conditions joined with `&&` (AND) or `||` (OR)
- Visibility values: `"PUBLIC"`, `"PROTECTED"`, `"PRIVATE"`
- Row status values: `"NORMAL"`, `"ARCHIVED"`
...
```json
{
  "resources": [{ "name": "resources/1" }, { "name": "resources/2" }]
}
```
    ...
    },
    {
      ...
      "paragraphNode": {
        "children": [
          ...
          {
            "type": "BOLD",
            "boldNode": { "children": [{ "type": "TEXT", "textNode": { "content": "bold" } }] }
          },
